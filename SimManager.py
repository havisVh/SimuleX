...
def SaveSimulation(simulation, env="int", path="/"):
    onbjType = simulation["type"].split("_")
    if onbjType[0] == "numarray":
        if onbjType[1] == "increment":
            if env == 'int':
                ...
                with open(path + simulation["title"] + ".py", "w") as file:
                    file.write(line1)
                ...
                ctx = ""
                with open(path+simulation["title"] + ".csv", "w") as file:
                    t = 0
                    file.write("t,x,y\n")
                    for i in range(int(round(simulation["time"]/simulation["increment"]))):
                        t = round(t, 2)
                ...
                with open(path+simulation["title"] + ".json", "w") as file:
